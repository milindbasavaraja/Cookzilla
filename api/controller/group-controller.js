...
export const getAllNonJoinedGroups = (req, res) => {
  ...
  jwt.verify(token, "userLoggedInCookzilla", (error, userInfo) => {
    ...
    const getAllNonJoinedGroupsQuery =
      "SELECT distinct g.gName, g.gCreator,g.gDesc FROM `Group` g, GroupMembership gm WHERE g.gName = gm.gName AND g.gCreator = gm.gCreator AND (g.gName,g.gCreator) NOT IN (SELECT  gm1.gName , gm1.gCreator FROM GroupMembership gm1 WHERE gm1.memberName=?)";

    db.query(getAllNonJoinedGroupsQuery, [userInfo.id], (error, data) => {
      if (error) {
        console.log(error);
...
export const getAllJoinedGroups = (req, res) => {
  const token = req.cookies.access_token_cookzilla;
  ...
  jwt.verify(token, "userLoggedInCookzilla", (error, userInfo) => {
    if (error) res.status(403).json("Token is not valid");
    const getAllNonJoinedGroupsQuery =
      "SELECT distinct g.gName, g.gCreator,g.gDesc FROM `Group` g, GroupMembership gm WHERE g.gName = gm.gName AND g.gCreator = gm.gCreator AND (g.gName,g.gCreator) IN (SELECT  gm1.gName , gm1.gCreator FROM GroupMembership gm1 WHERE gm1.memberName=?)";
